function [imagewithpassencrypt] = encode2() 

clc
texthidepass = evalin('base','texthidepass')
passwordencrypt = evalin('base','passwordencrypt')
pathpasscover = evalin('base','pathpasscover')
im1 = imread(pathpasscover);
fid = fopen('message.txt','wb');
fwrite(fid, texthidepass+"%", 'char');
...
im2 = im1;
bool_im = zeros(l,w,b);
pass = convertStringsToChars(passwordencrypt);
password = pass;
assignin('base','password',pass)
...
imwrite(im2, 'stegotextpassword.png');
imagewithpassencrypt = '../stegotextpassword.png';
assignin('base','imagewithpassencrypt',imagewithpassencrypt)
figure;
